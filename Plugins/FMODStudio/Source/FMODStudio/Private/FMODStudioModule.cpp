...
public:
    ...
    // <ANS - HVALDIVIESO>
    virtual void SetListenerPositionWithOverride(int ListenerIndex, UWorld* World, const FTransform& ListenerTransform, const FVector& AttenuationOverridePosition, float DeltaSeconds) override;
    // <ANS - HVALDIVIESO>
...
void FFMODStudioModule::UpdateWorldListeners(UWorld *World, int *ListenerIndex)
{
    ...
    for (auto Iterator = GEngine->GetLocalPlayerIterator(World); Iterator; ++Iterator)
    {
        ...
        if (LocalPlayer && LocalPlayer->PlayerController)
        {
            ...
            // <ANS - HVALDIVIESO>
            FVector AttenuationOverridePosition;
            if (PlayerController->GetAudioListenerAttenuationOverridePosition(AttenuationOverridePosition))
            {
                SetListenerPositionWithOverride(*ListenerIndex, World, ListenerTransform, AttenuationOverridePosition, DeltaSeconds);
            }
            else
            {
...
// <ANS - HVALDIVIESO>
void FFMODStudioModule::SetListenerPosition(const int ListenerIndex, UWorld* World, const FTransform& ListenerTransform, const float DeltaSeconds)
{
    SetListenerPositionWithOverride(ListenerIndex, World, ListenerTransform, ListenerTransform.GetLocation(), DeltaSeconds);
}
// <ANS - HVALDIVIESO>

// Partially copied from FAudioDevice::SetListener
void FFMODStudioModule::SetListenerPositionWithOverride(int ListenerIndex, UWorld* World, const FTransform& ListenerTransform, const FVector& AttenuationOverridePosition, float DeltaSeconds)
{
    ...
    if (System && ListenerIndex < MAX_LISTENERS)
    {
        ...
        // <ANS - HVALDIVIESO>
        // If the attenuation override location is different from the listener location, we assume that it was overriden.
        const bool bAttenuationOverriden = ListenerTransform.GetLocation() != AttenuationOverridePosition;
        // For the audio volume system implementation we need to provide the correct location.
        const FVector AudioVolumeRelevantLocation = bAttenuationOverriden ? AttenuationOverridePosition : ListenerTransform.GetTranslation();
        ...
        AAudioVolume *Volume = World->GetAudioSettings(/*<ANS - HVALDIVIESO>*/AudioVolumeRelevantLocation/*<ANS - HVALDIVIESO>*/, nullptr, InteriorSettings);
        // <ANS - HVALDIVIESO>
        ...
        Attributes.position = FMODUtils::ConvertWorldVector(/*<ANS - HVALDIVIESO>*/ListenerTransform.GetTranslation()/*<ANS - HVALDIVIESO>*/);
        ...
        // <ANS - HVALDIVIESO>
        if (bAttenuationOverriden)
        {
            // UE Vector (Z Up) needs to be converted to FMOD Vector matrix (X: Up).
            const FMOD_VECTOR AttenuationPosition = FMODUtils::ConvertWorldVector(AttenuationOverridePosition);
            verifyfmod(System->setListenerAttributes(ListenerIndex, &Attributes, &AttenuationPosition));
        }
        else
        {
